var tabla_videos;
tbody = $("#tabla_videos").find('tbody');
tbody.attr("hidden", false);
tbody.fadeOut(1);

$(document).ready(() => {
   tabla_videos = $('#tabla_videos').DataTable({
      "responsive": true,
      // scrollX: true,
      language: {
   ...
   function actualizarStatusRegistros() {
      ...
      query = "UPDATE video SET vid_status='0', vid_order=1000000 WHERE vid_id IN (";
...
function rellenarFiltroClientes(ajaxResponse) {
   ...
   tabla_videos.columns(0).search(cliente).draw();
   enlistarOrden();
   tbody.fadeIn();
}
card_body.click((e) => {
