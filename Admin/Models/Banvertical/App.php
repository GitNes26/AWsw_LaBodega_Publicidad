...
if (isset($_FILES['input_archivo'])) {
   ...
   $directorio = "../../$path_archivos_panel/$ubicacion/Banverticales";
   ...
   if (move_uploaded_file($_FILES["input_archivo"]["tmp_name"],$destino)) {
      $ruta = "$path_archivos_panel/$ubicacion/Banverticales/$ubicacion-$nombre_archivo";
   } else {
      $ruta = "";
      $tipo = "";
...
//PETICIONES
if ($accion == 'mostrar_Banverticales_cliente') {
   $Banvertical->mostrarBanverticalesPorCliente($id_cliente);
}
...
if ($accion == 'editar_banvertical') {
   ...
   $Banvertical->editarBanvertical($id,$ubicacion,$ruta,$tipo,$fecha_inicial,$fecha_final,$status,$asignar_orden);
}
...
if ($accion == "actualizar_orden") {
   $Banvertical->actualizarOrden($id,$orden);
}
...
function moverArchivo($src_archivo,$ubicacion,$ruta) {
   ...
   if ($src_archivo == "" || $src_archivo == null) {
      ...
      $ruta = trim(end($path));
      // var_dump($ruta);
      $cantidad = (int)$Banvertical->contarRegistrosConLaMismaRuta($ruta);
      // var_dump($cantidad);
      if ($cantidad > 0) {
         @copy($path_actual,$destino);
      } else {
         if (rename($path_actual,$destino)) {
            $ruta = "$path_archivos_panel/$ubicacion/Banverticales/$nombre_archivo";
         } else {
            print(error_get_last());
         }
      }
