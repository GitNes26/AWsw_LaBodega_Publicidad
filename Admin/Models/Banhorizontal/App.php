...
if (isset($_FILES['input_archivo'])) {
   ...
   $directorio = "../../$path_archivos_panel/$ubicacion/Banhorizontal";
   ...
   if (move_uploaded_file($_FILES["input_archivo"]["tmp_name"],$destino)) {
      $ruta = "$path_archivos_panel/$ubicacion/Banhorizontal/$ubicacion-$nombre_archivo";
   } else {
      $ruta = "";
      $tipo = "";
...
if ($accion == 'editar_banhorizontal') {
   ...
   $Banhorizontal->editarBanhorizontal($id,$ubicacion,$ruta,$tipo,$fecha_inicial,$fecha_final,$status);
}
...
function moverArchivo($src_archivo,$ubicacion,$ruta) {
   ...
   if (rename($path_actual,$destino)) {
      $ruta = "$path_archivos_panel/$ubicacion/Banhorizontal/$nombre_archivo";
   } else {
      print(error_get_last());
   }
   return $ruta;
